...
changeSliderLabel(size);function determineDx(elem,size){var oldwidth=elem.offsetWidth;var windowwidth=document.getElementsByClassName("randomPizzas").offsetWidth;var oldsize=oldwidth / windowwidth;function sizeSwitcher(size){switch(size){case"1":return 0.25;case"2":return 0.3333;case"3":return 0.5;default:console.log("bug in sizeSwitcher");}}
var newsize=sizeSwitcher(size);var dx=(newsize-oldsize)*windowwidth;return dx;}
var pContainer=document.getElementsByClassName('randomPizzaContainer');var ranPizzaContainer=document.getElementsByClassName("randomPizzaContainer");function changePizzaSizes(size){var numPizzas=ranPizzaContainer.length;var dx;var newWidth;for(var i=0;i<numPizzas;i++){dx=determineDx(ranPizzaContainer[i],size);newWidth=(ranPizzaContainer[i].offsetWidth+dx)+'px';pContainer[i].style.width=newWidth;}}
...
var items=document.getElementsByClassName('mover');function updatePositions(){frame++;window.performance.mark("mark_start_frame");var currentScrollTop=document.body.scrollTop / 1250;var phases=[Math.sin(currentScrollTop+0),Math.sin(currentScrollTop+1),Math.sin(currentScrollTop+2),Math.sin(currentScrollTop+3),Math.sin(currentScrollTop+4)];var itemsLength=items.length;var move;for(var i=0;i<itemsLength;i++){move=(items[i].basicLeft+100*phases[i%5])-window.innerWidth/2;items[i].style.transform='translateX('+move+'px)';}
...
window.addEventListener('scroll',updatePositions);window.addEventListener('resize',reMakeBackgroundPizzas);document.addEventListener('DOMContentLoaded',makeBackgroundPizzas);var cols=8;var s=256;var movPizzas=document.getElementById("movingPizzas1");function makeBackgroundPizzas(){var elem;var numberOfPizzas=maxPizzasToGenerate();for(var i=0;i<numberOfPizzas;i++){elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.style.height="100px";elem.style.width="73.333px";elem.basicLeft=(i%cols)*s;elem.style.top=(Math.floor(i / cols)*s)+'px';movPizzas.appendChild(elem);}
updatePositions();}
function reMakeBackgroundPizzas(){var children=movPizzas.getElementsByClassName('mover');var numChildren=children.length;for(var i=0;i<numChildren;i++){movPizzas.removeChild(children[i]);}
makeBackgroundPizzas();}
function maxPizzasToGenerate(){var numRows=window.innerHeight/256;return Math.ceil(numRows)*cols;;}